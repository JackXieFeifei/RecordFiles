...
class AndroidBuilder:
    ...
    def load_environment(self):
        ''' Load environment of Android SDK, NDK and ANT from 'local.properties'
        '''
        self.env = {}
        ...
        print 'ant.dir = ' + self.env['ant.dir']
    ...
    def ant_build(self, build_param):
        ''' Use android tools to build and generate an android APK
        '''
        ant_path = os.path.join(self.env['ant.dir'], 'ant')
        command = '%s %s' % (ant_path, build_param)
        
        if os.system(command) != 0:
            raise Exception('Ant build failed!')
    ...
    def do_action(self, action):
        ...
        elif 'ant-build' in action:
            self.ant_build(action['ant-build'])
